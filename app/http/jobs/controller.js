...
class NewJobsController {
    async create(req, res) {
        try {
            const { jobTitle, jobLocation, noOfVacancies, experience, age, salaryFrom, salaryTo, jobType, status, description, department, startDate, expireDate } = req.body;

            if (!jobTitle || !jobLocation || !noOfVacancies || !experience || !age || !salaryFrom || !salaryTo || !jobType || !status) {
                return BadRequest(res, "All required fields must be provided.");
            }

            const newJob = new AddNewJobs({
                jobTitle, jobLocation, noOfVacancies, experience, age, salaryFrom, salaryTo, jobType, status, description, department, startDate, expireDate
            });
